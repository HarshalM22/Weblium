...
const Builder: React.FC = () => {
  const { prompt, steps, currentStep, files, isBuilding, startBuild } = useBuild();
              ...
              {selectedTab === 'preview' ? (
                <div className="h-full flex items-center justify-center">
                  <div className="text-center p-8 bg-slate-800/50 rounded-lg border border-slate-700 max-w-md">
                    ...
                    <h3 className="text-xl font-semibold text-white mb-2">Website Preview</h3>
                    <p className="text-slate-300 mb-4">Preview will be available once the build process is complete.</p>
                    {currentStep >= steps.length && (
                      <button className="bg-gradient-to-r from-purple-500 to-blue-500 text-white px-4 py-2 rounded-md shadow-lg hover:shadow-purple-500/20 transition-all">
                        View Live Preview
                      </button>
                    )}
                  </div>
      ...
      <div key={item.id} style={{ marginLeft: `${level * 12}px` }}>
        <div 
          className={`flex items-center py-1.5 px-2 rounded-md cursor-pointer ${
            isSelected ? 'bg-purple-500/20 text-white' : 'text-slate-300 hover:bg-slate-800 hover:text-white'
          } transition-colors`}
